...
#               ${2} = list of hosts to install Splunk Enterprise 
...
# ===========================================================
#
stopSplunk="sudo su - splunk -c '/opt/splunk/bin/splunk stop'"
untarSplunk="tar -zxvf /tmp/${1} -C /opt && chown -R splunk:splunk /opt/splunk"
startSplunk="sudo su - splunk -c '/opt/splunk/bin/splunk start --accept-license --answer-yes --no-prompt'"
for HOST in $(< $2); do
    scp -r "${1}" $HOST:/tmp
    ssh $HOST "$stopSplunk"
