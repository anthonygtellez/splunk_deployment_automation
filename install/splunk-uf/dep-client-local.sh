...
# 		such as configuring the deployment client & client name based on a list of
# 		possible values. 
#
# Parameters:	${1} = path to splunk install .tgz file
#		Clientname (name to set in deploymentclient.conf)
#
# Example usage: $ bash advancedufinstall.sh splunk-6.3.2-aaff59bb082c-Linux-x86_64.tgz
#
...
read -p "set client name for the deployment client " clientname
printf "%b" "\n"
mkdir -p /opt/splunkforwarder/etc/apps/${clientname}_deploymentclient/local
cat >/opt/splunkforwarder/etc/apps/${clientname}_deploymentclient/local/deploymentclient.conf<<EOF
[deployment-client]
